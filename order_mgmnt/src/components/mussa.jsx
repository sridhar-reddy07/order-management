...
const Mussa = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await fetch('http://137.184.75.176:5000/mussaList');
        if (!response.ok) {
  ...
  useEffect(() => {
    ...
    axios.get(`http://137.184.75.176:5000/mussaList?search=${search}`)
      .then((response) => {
...
function cleanFileName(url) {
  ...
  const downloadPDF = () => {
    ...
    doc.save(`Mussa_orders_${moment().format('YYYY-MM-DD')}.pdf`);
  };
