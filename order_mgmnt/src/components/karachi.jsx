...
const Karachi = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await fetch('http://137.184.75.176:5000/karachiList');
        if (!response.ok) {
  ...
  useEffect(() => {
    ...
    axios.get(`http://137.184.75.176:5000/karachiList?search=${search}`)
      .then((response) => {
  ...
  const downloadPDF = () => {
    ...
    doc.save(`Karachi_orders_${moment().format('YYYY-MM-DD')}.pdf`);
  };
