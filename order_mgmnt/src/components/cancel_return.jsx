...
const Cancel_Return = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await fetch('http://137.184.75.176:5000/returnList');
        if (!response.ok) {
  ...
  useEffect(() => {
    ...
    axios.get(`http://137.184.75.176:5000/returnList?search=${search}`)
      .then((response) => {
...
function cleanFileName(url) {
  ...
  const downloadPDF = () => {
    ...
    doc.text('cancel_and_return_Orders', 14, 22);
    ...
    doc.save(`cancel_and_return_orders_${moment().format('YYYY-MM-DD')}.pdf`);
  };
