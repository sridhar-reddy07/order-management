...
const Embroidory = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await fetch('http://137.184.75.176:5000/embroidoryList');
        if (!response.ok) {
  ...
  useEffect(() => {
    ...
    axios.get(`http://137.184.75.176:5000/embroidoryList?search=${search}`)
      .then((response) => {
  ...
  const downloadPDF = () => {
    ...
    doc.text('Embroidery Orders', 14, 22);
    ...
    doc.save(`Embroidery_orders_${moment().format('YYYY-MM-DD')}.pdf`);
  };
                       ...
                       <p><strong>Files Uploaded:</strong></p>
                        {isAdmin ? <a
                            ...
                            {order.files && order.files.length > 0 ? (
                              order.files
                                .filter((file) => file.fileUrl) // Filter out any files without a URL
                                .map((file, idx) => {
