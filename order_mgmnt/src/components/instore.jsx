...
const Instore = () => {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        const response = await fetch('http://localhost:5000/instoreList');
        if (!response.ok) {
  ...
  useEffect(() => {
    ...
    axios.get(`http://localhost:5000/instoreList?search=${search}`)
      .then((response) => {
            ...
            return (
              <React.Fragment key={index}>
                <tr>
                  ...
                  <td>
                    <select
                      value={order.orderStatus || ""} // Ensure it defaults to an empty string if undefined
